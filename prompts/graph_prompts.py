...
"""
    + """\
1. **Trust session state for files and profiling.** **Session workspace** (`data_profile` in state) is a **list** of per-file profiles, refreshed automatically before each turn: each entry has `file`, `head` (5 rows), `time_columns`, and `columns` with `dtype`, `null_pct`, `numeric_pct`, `cardinality`, `possible_categorical`, and `continuous_stats` (min/max/mean/median) for numeric columns. There is no separate list/read/profile tool.
