...
3. Parse the JSON result: `status`, `stdout`, `stderr`, `code_execution_result` (when present: sandbox stdout/stderr, `outputs`, `plots`), `code_violation` (active keys: `semgrep`, `e2b`, `codegen_failure`, `node_error`), `outputs`, `plots`, and optionally `code` on failure. On E2B failure, use `code_violation.e2b` (and `stderr` / `code_execution_result.stderr`) when refining `requirements` for another `coding_tool` call.
