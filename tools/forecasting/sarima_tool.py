...
def data_validation(
    *,
    ...
    """
    Read the input file from the session workspace and ensure the required columns exist.

    Returns a two-column DataFrame ``[date_column, target_column]``. Any of these
    failures break the pipeline: bad filename, unsupported extension, missing
    file, missing required columns, or fewer than ``MIN_OBS`` rows.
    """
    ...
    missing = [c for c in (date_column, target_column) if c not in df.columns]
    if missing:
        raise SarimaToolError(
            "missing_required_columns",
            f"Required columns missing in '{name}': {missing}. Available: {list(df.columns)[:20]}",
            "data_validation",
        )
...
def prepare_time_index(
    ...
    *,
    ...
    """
    Build a numeric ``pd.Series`` indexed by a regular ``DatetimeIndex``.

    Steps:
    1. Parse the date column (any unparseable value breaks the pipeline).
    2. Sort by date and reject duplicate timestamps.
    3. Run ``pd.infer_freq`` — if it cannot find a regular cadence, the tool
       returns an error so the orchestrator can decide whether to ask the user
       or resample upstream. We do not silently invent a frequency.

    The returned series has its frequency attached to the index so statsmodels
    SARIMAX can produce future timestamps for the forecast.
    """
...
def check_target_values(series: pd.Series) -> pd.Series:
    ...
    if missing > 0:
        raise SarimaToolError(
            ...
            f"Target column has {missing} missing values. Clean/impute the data before fitting ARIMA.",
            "check_target_values",
        )
...
def fit_model(series: pd.Series, spec: dict) -> Any:
    ...
    if spec["seasonal_order"] is not None and spec["seasonal_period"]:
        seasonal_order = tuple(spec["seasonal_order"]) + (int(spec["seasonal_period"]),)
    else:
        # SARIMAX's "no seasonality" signal.
    ...
    with langfuse.start_as_current_observation(
        ...
        try:
            model = SARIMAX(
                ...
                # Relax these so user-supplied near-non-stationary orders still fit;
                # diagnostics will flag any practical issues.
...
def extract_fit_quality(fit: Any) -> dict:
    ...
    # ``df_model`` excludes the variance parameter; add 1 so AICc matches the textbook formula.
    ...
    if nobs - k - 1 > 0:
        ...
        # Sample is too small for the AICc correction; fall back to AIC so downstream JSON stays clean.
        aicc = aic
...
def diagnose_residuals(fit: Any, seasonal_period: Optional[int]) -> dict:
    ...
    Bad results never fail the tool — they only attach human-readable warnings
    that the LLM interpretation step will translate into business language.
    """
    ...
    # Hyndman-Athanasopoulos: for seasonal models use ``2 * m`` lags, else min(10, n/5).
    ...
    # Residual mean should sit within ~2 standard errors of zero; otherwise the
    # forecast is biased on average.
    ...
    status = "warn" if warnings_out else "pass"
