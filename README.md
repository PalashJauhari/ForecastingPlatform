...
- **Six bound tools** — `build_codegen_requirement`, `code_pipeline`, `sarima_tool`, `ask_user`, `write_scratchpad`, `write_todos`. The orchestrator uses **`make_llm`** then **`llm.bind_tools(TOOLS)`** (`graph/graph.py`); codegen paths use **`make_llm(..., output_schema=...)`** without tools.
