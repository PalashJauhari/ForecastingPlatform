...
- **Workspace profiling (`data_profile`)** — **ProfileSavedData** nodes run once per **prep** and again after **`RunTools`** (**ProfileSavedData_PostTools**) before **`Orchestrator`**. They update **`data_profile`** in state (**`profiling_data.profile_session_workspace`**).
...
**Graph nodes (non-middleware):** `BeginTurn`, `ProfileSavedData`, `SummariseConversationalSummary`, `SelectPlannerSkills`, `Planner`, `SelectOrchestratorSkills`, `Orchestrator`, `RunTools`, `ProfileSavedData_PostTools`, `TodoCompletionGate`, `FinalAnswer`.
...
|------|-----------|--------------|
| **code_pipeline** | Structured **`task`** (`requirements`, `input` basenames, `output` basenames), optional **`data_profile`**, **`previous_code_violation`** | Pipeline: structured codegen → Semgrep → LLM judge (both always run when code is non-empty) → save `pipeline_run.py` → `run_pipeline_sandboxed.py` (session workspace + **`run_<tool_call_id>/`**, optional IO allowlist JSON). **Only image-producing tool**; API exposes plots via **`images`**. Tool JSON: **`stdout`** / **`stderr`** / **`code_violation`** / **`plots`**, and **`code`** when safety or runtime issues require it (**`task`** not repeated in JSON). Retry with **`previous_code_violation`**. |
